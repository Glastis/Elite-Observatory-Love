...
local function format_gravity(gravity_ms2)
    if not gravity_ms2 or gravity_ms2 <= 0 then return constants.UNKNOWN_TEXT end
...
local function badge_item_for(badge_def)
    ...
    local width = badge.measure(badge_def.label, font, {
...
local FOOTER_BUILDERS = {
    {
        active = function(body)
            if body.is_star or is_currently_mapped(body) or not body.worth_mapping then return false end
            return ((body.potential_max or 0) - (body.current_value or 0)) > 0
        end,
        build = function(body)
            local delta = (body.potential_max or 0) - (body.current_value or 0)
            return { label = string.format(FOOTER_MAP_FOR, format_value_str(delta)), color_key = "accent" }
        end,
    },
    {
...
local function build_footer(body)
    for _, builder in ipairs(FOOTER_BUILDERS) do
        if builder.active(body) then return builder.build(body) end
    end
    return nil
end
...
local function card_height(card)
    ...
    local badges_h = badge.rows_height(card.badge_rows, BADGE_ROW_H, BADGE_GAP_Y)
    if badges_h > 0 then h = h + CARD_INTERNAL_GAP + badges_h end
    if card.footer then h = h + FOOTER_GAP + FOOTER_H end
    return h
end

local function panel_opts_for(card)
    local accent = card.is_high_value and theme.colors.accent or theme.colors.accent_rule
    return {
        left_accent = accent,
        left_accent_w = card.is_high_value and 3 or 2,
    }
end
...
local function draw_card_distance(card, x, y, w)
    if not card.distance_text or card.distance_text == constants.UNKNOWN_TEXT then return end
...
local function draw_footer(card, x, y, w)
    ...
    text.draw(fitted, x, y, {
        font = font, color = theme.colors[card.footer.color_key], letter_em = 0.04,
    })
end
...
function CARD_VIEW.draw(view_state, x, y, w, h, settings, hide_system, sort_mode, hide_scanned)
    return card_view.draw(view_state, x, y, w, h, {
        ...
        empty_message = "(no bodies above the value threshold)",
